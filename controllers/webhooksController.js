...
module.exports = {
  ...
  webhook: async (req, res) => {
    try {
      ...
      let event;
      try {
        event = await stripe.webhooks.constructEvent(req.rawBody, sig, stripe_webhook_key);
      } catch (err) {
        // console.log(err);
  ...
  webhookConnectedAccount: async (req, res) => {
    ...
    try {
      ...
      let event;
      try {
        event = await stripe.webhooks.constructEvent(req.rawBody, sig, stripe_connect_webhook_key);
      } catch (err) {
        // console.log(err);
      ...
      switch (event.type) {
        case 'account.updated': {
          ...
          if (Object.keys(account.requirements.currently_due).length
          || Object.keys(account.requirements.pending_verification).length) {
            ...
            if (!wasStripeConnected && profile.isStripeConnected) {
              Notifier.notifyUser(profile.user, 'profileUpdated', 'STRIPE_CONNECTED', '', '', '', null, null);
            }
          }
        ...
        case 'payment_intent.succeeded': {
          ...
          if (!paymentIntent.metadata.orderId && paymentIntent.invoice) {
            return res.status(200).json({
              received: true, message: SUBSCRIPTION_ALREADY_HANDLED_BY_INVOICE_PAID,
            });
          }
          if (!paymentIntent.metadata.orderId && !paymentIntent.invoice) {
            return res.status(404).json({ received: false, message: METADATA_NOT_FOUND });
          }
          ...
          if (responseObj.statusCode === 404) {
            return res.status(responseObj.statusCode).json(responseObj.responseBody);
          }
          break;
        }
        case 'invoice.paid': {
          ...
          const { orderId, origin } = invoicePaidIntent.lines.data[0].metadata;
